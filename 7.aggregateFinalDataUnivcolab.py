...
if not os.path.isdir(conf.preProcess.FinalAggregatedDataDirUniversityColab):
    # Create dir
    os.makedirs(conf.preProcess.FinalAggregatedDataDirUniversityColab, exist_ok=True)
...
patFiles = os.listdir(
    os.path.join(conf.preProcess.outputNiftiPatientDirFinalResampledStacked4D, "train", "labels")
)

for fileNr, patFile in enumerate(patFiles):
    # Get patient string
    patient = patFile.split("_")[0]
    # Copy the file to conf.preProcess.finalAggregatedDataDirUniversityColab
    shutil.copyfile(
        os.path.join(
            conf.preProcess.outputNiftiPatientDirFinalResampledStacked4D, "train", "labels", patFile
        ),
        os.path.join(conf.preProcess.FinalAggregatedDataDirUniversityColab, patFile),
    )
    ...
    for fileName in conf.preProcess.ImageFileNameStack:
        # Get full file path
        filePathSource = os.path.join(
            conf.preProcess.outputNiftiPatientDirFinalResampled, patient, fileName
        )  # Note that we get the files from the image folder
        # Define new file name without spaces
        newFileName = fileName.replace(" ", "_")
        # Rename file to include patient string and copy to conf.preProcess.finalAggregatedDataDirUniversityColab
        filePathTarget = os.path.join(
            conf.preProcess.FinalAggregatedDataDirUniversityColab, patient + "_" + newFileName
        )
        # Copy the file
...
root_directory = conf.preProcess.FinalAggregatedDataDirUniversityColab
output_json_file = os.path.join(root_directory, conf.preProcess.site + ".json")
# Create json file if there are X number of files in the finalAggregatedDataDirUniversityColab directory
