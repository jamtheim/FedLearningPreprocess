...
class commonConfigClass:
    ...
    if preProcess.study == "Validation":
        ...
        base.dataFolderMICE = os.path.join(
            base.workFolder, "ExtractStructuresData/Mice Batch/20230920/#Validation"
        )
    elif preProcess.study == "Treatment":
        preProcess.inputDicomPatientDir = base.treatmentStudyFolder
        base.dataFolderMICE = os.path.join(
            base.workFolder, "ExtractStructuresData/Mice Batch/20230920/#Treatment"
        )

    # Define output directory for Nifti data
    preProcess.outputNiftiPatientDir = os.path.join(
        base.workFolder, "ExtractStructuresData", preProcess.study
    )
    # Corresponding folder for fused final data (used in 2.fuseDataAndAnonomise.py)
    preProcess.outputNiftiPatientDirFinal = os.path.join(
        base.workFolder, "ExtractStructuresData", preProcess.study, "FinalAnon"
    )
    # Corresponding folder for fused final and resampled data (used in 3.resampleAnonData.py)
    preProcess.outputNiftiPatientDirFinalResampled = os.path.join(
        base.workFolder, "ExtractStructuresData", preProcess.study, "FinalAnonResampled"
    )
    # Corresponding folder for fused final, resampled and stacked data (used in 4.stackData.py)
    preProcess.outputNiftiPatientDirFinalResampledStacked = os.path.join(
        base.workFolder, "ExtractStructuresData", preProcess.study, "FinalAnonResampledStacked"
    )

    # Corresponding folder for fused final, resampled and stacked data (used in 4.stackData.py)
    preProcess.outputNiftiPatientDirFinalResampledStacked4D = os.path.join(
        base.workFolder, "ExtractStructuresData", "FinalAggregatedUniversityColab", preProcess.study
    )
    ...
    preProcess.finalAggregatedDataDirScaleout = os.path.join(
        base.workFolder, "ExtractStructuresData", preProcess.finalAggregatedDataScaleout
    )
    ...
    preProcess.finalAggregatedDataDirIfusion = os.path.join(
        base.workFolder, "ExtractStructuresData", preProcess.finalAggregatedDataIfusion
    )

    preProcess.FinalAggregatedDataUniversityColab = "FinalAggregatedUniversityColabFinished"
    preProcess.FinalAggregatedDataDirUniversityColab = os.path.join(
        base.workFolder, "ExtractStructuresData", preProcess.FinalAggregatedDataUniversityColab
    )
    ...
    preProcess.faceMaskAnonDistance = (
        0,
        120,
        -40,
    )  # Number of voxels to move from center to define center of face mask
