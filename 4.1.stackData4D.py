...
# Description: Stack all Nifti data and structures as a multi channel format
# *********************************************************************************
...
def patLargeDataLoop(patNr, patient):
    ...
    """
    # Stack Nifti data
    convertData.stackNiftiData4D(
        patNr,
        patient,
        conf.preProcess.outputNiftiPatientDirFinalResampled,
        conf.preProcess.outputNiftiPatientDirFinalResampledStacked4D,
    )
...
if os.path.isdir(conf.preProcess.outputNiftiPatientDirFinalResampledStacked4D):
    # Remove the folder
    shutil.rmtree(conf.preProcess.outputNiftiPatientDirFinalResampledStacked4D)

# Loop over all patient folders
patFolders = os.listdir(conf.preProcess.outputNiftiPatientDirFinal)
# Only include folder
patFolders = [
    x for x in patFolders if os.path.isdir(os.path.join(conf.preProcess.outputNiftiPatientDirFinal, x))
]
# Make sure the output Nifti directory exists
if not os.path.isdir(conf.preProcess.outputNiftiPatientDirFinalResampledStacked4D):
    # Create dir
    os.makedirs(conf.preProcess.outputNiftiPatientDirFinalResampledStacked4D, exist_ok=True)

# Set number of CPU threads to use
nrCPU = 20
# Init parallell job for resampling the Nifti data
patInfo = Parallel(n_jobs=nrCPU, verbose=10)(
    delayed(patLargeDataLoop)(patNr, patient) for patNr, patient in enumerate(patFolders)
)
