...
class convertDataMethodsClass:
    ...
    def stackNiftiData(self, i_subject, subject, dataInBasePath, dataOutBasePath):
        ...
        subjectFiles = [
            f for f in os.listdir(subjectFolderPath) if os.path.isfile(os.path.join(subjectFolderPath, f))
        ]
        ...
        for index, imageFile in enumerate(conf.preProcess.ImageFileNameStack):
            ...
            if conf.preProcess.createNNUnetData == True:
                ...
                shutil.copyfile(
                    os.path.join(subjectFolderPath, imageFile),
                    os.path.join(nnUnetDataOutFolderPathData, subject + "_000" + str(index) + ".nii.gz"),
                )
                # Assert that index < 9
        ...
        for GTFile in conf.preProcess.GTStructureFileName.keys():
            ...
            assert (
                GTData.shape[0] == stackedVolume.shape[0]
            ), "Input GTData must have same size as stackedVolume for the first dimension"
            assert (
                GTData.shape[1] == stackedVolume.shape[1]
            ), "Input GTData must have same size as stackedVolume for the second dimension"
            assert (
                GTData.shape[2] == stackedVolume.shape[2]
            ), "Input GTData must have same size as stackedVolume for the third dimension"
    ...
    def stackNiftiData4D(self, i_subject, subject, dataInBasePath, dataOutBasePath):
        """
        Stack Nifti data to multi channel Nifti file. ALso handles GT data.
        ...
        for GTFile in conf.preProcess.GTStructureFileName.keys():
            ...
            GTData = nibabel.load(GTFilePath)
            GTDataAffine = GTData.affine

            # change dtype to int8
            # GTData = nibabel.Nifti1Image(GTData.get_fdata().astype(np.int8), GTData.affine)
            GTData = GTData.get_fdata().astype(np.uint8)

            # Assert GTData is 1 or 0
            assert np.all(np.isin(GTData, [0, 1])), "GT data must be binary"
            ...
            # check that the gt data is int8
            assert GTData.dtype == np.uint8, "GT data must be uint8"

            GT_files.append(GTData)
            # GT_files_affine.append(GTDataAffine)

        GTVolumeCombined = np.stack(GT_files, axis=-1)

        GTVolumeCombined = nibabel.Nifti1Image(GTVolumeCombined, GTDataAffine)
        outputFilename = os.path.join(GTOutFolderPath, f"{subject}_structures_seg.nii.gz")
        nibabel.save(GTVolumeCombined, outputFilename)
    ...
    def faceMaskAnon(
        self, imgData, boundingBoxData, subjectFolderPath, faceMaskAnonDist, faceMaskAnonSize, subject
    ):
        """
        ...
        reSampledOrientData = nibabel.processing.resample_to_output(
            orientData, voxel_sizes=conf.preProcess.voxelSize, order=0, mode="constant", cval=0.0
        )
        ...
        reSampledCropOrientData = self.cropImageFromMask(
            reSampledOrientData, boundingBoxData, conf.preProcess.marginCropVoxel, subject
        )
        # This must now be the same size as imgData, assert
        assert (
            reSampledCropOrientData.shape == imgData.shape
        ), "Input reSampledOrientData must have same size as imgData for all dimensions"
        ...
        faceMaskCenter = (
            centerOfMass[0] + faceMaskAnonDist[0],
            centerOfMass[1] + faceMaskAnonDist[1],
            centerOfMass[2] + faceMaskAnonDist[2],
        )
    ...
    def processNiftiData(self, i_subject, subject, dataInBasePath, dataOutBasePath):
        ...
        # Resample to chosen spatial resolution, using nearest neighbour interpolation as this is a mask.
        reSampledBBData = nibabel.processing.resample_to_output(
            BBData, voxel_sizes=conf.preProcess.voxelSize, order=0, mode="constant", cval=0.0
        )
        ...
        for subjectFile in subjectFiles:
            ...
            # Perform resamling
            reSampledSubjectData = nibabel.processing.resample_to_output(
                subjectData,
                voxel_sizes=conf.preProcess.voxelSize,
            ...
            assert (
                reSampledSubjectData.shape == reSampledBBDataNp.shape
            ), "Input reSampledSubjectData must have same size as reSampledBBDataNp for all dimensions"
            # Crop volume with respect to the bounding box structure file including marginal space in voxels
            reSampledCutSubjectData = self.cropImageFromMask(
                reSampledSubjectData, reSampledBBDataNp, conf.preProcess.marginCropVoxel, subject
            )
            # Deface the data if option is set and condition is met
            if conf.preProcess.defaceData == True:
                reSampledCutSubjectData = self.faceMaskAnon(
                    reSampledCutSubjectData,
                    reSampledBBDataNp,
                    subjectFolderPath,
                    conf.preProcess.faceMaskAnonDistance,
                    conf.preProcess.faceMaskAnonSize,
                    subject,
                )
            # Zero pad data to desired size
            reSampledCutPadSubjectData = self.padAroundImageCenter(
                reSampledCutSubjectData, conf.preProcess.paddedSize, subject
            )
    ...
    def DicomRT2Nifti(self, i_subject, subject, dataInBasePath, dataOutBasePath):
        ...
        if conf.preProcess.study == "Validation":
            subjectCTFolderPath = self.selectSubfolder_CT(
                os.path.join(subjectFolderPath, "CT"), "Skalle__2_0__H30s", "iMAR"
            )
        ...
        subjectStructFile = self.getRTStructFile(
            os.path.join(
                subjectFolderPath, conf.preProcess.RTstructFolderFirst, conf.preProcess.RTstructFolderSecond
            )
        )
        subjectStructFilePath = os.path.join(
            subjectFolderPath,
            conf.preProcess.RTstructFolderFirst,
            conf.preProcess.RTstructFolderSecond,
            subjectStructFile,
        )
        ...
        subjectStructList = [
            x
            for x in subjectStructList
            if any(word.lower() in x.lower() for word in conf.preProcess.structureNameKeywords)
        ]
        # Run the list again but this time to exclude any structures that contain any of the keywords defined in exclude list
        subjectStructList = [
            x
            for x in subjectStructList
            if not any(word.lower() in x.lower() for word in conf.preProcess.structureNameExclude)
        ]
        # print(subjectStructList)
        ...
        for structNr, currStruct in enumerate(subjectStructList):
            try:
                ...
                if (
                    currStruct in conf.preProcess.bodyStructureName1
                    or currStruct in conf.preProcess.bodyStructureName2
                ):
                    # print(subject)
                    dcmrtstruct2nii(
                        subjectStructFilePath,
                        subjectCTFolderPath,
                        subjectOutFolderPath,
                        structures=currStruct,
                        gzip=True,
        ...
        if patDelete == True:
            ...
            if nrGTVT == 1:
                ...
                os.rename(
                    os.path.join(
                        subjectOutFolderPath, [x for x in subjectOutFolderContent if "GTVT" in x][0]
                    ),
                    os.path.join(subjectOutFolderPath, "mask_GTVT.nii.gz"),
                )
            else:
                # Throw message
            ...
            if nrCTVT == 1:
                ...
                os.rename(
                    os.path.join(
                        subjectOutFolderPath, [x for x in subjectOutFolderContent if "CTVT" in x][0]
                    ),
                    os.path.join(subjectOutFolderPath, "mask_CTVT.nii.gz"),
                )
            else:
                # Throw message
    ...
    def getRTStructFile(self, path):
        ...
        if len(structFile) == 0:
            raise Exception(
                "No RT structure file could be located. Make sure the file is located in the specified folder..."
            )
